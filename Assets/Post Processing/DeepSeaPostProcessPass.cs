...
[System.Serializable]
public class DeepSeaPostProcessPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        var materials = DeepSeaPostProcessingMaterials.Instance;
        if (materials == null)
        {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<DeepSeaEffectComponent>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            var material = materials.customEffect;
            // P.s. optimize by caching the property ID somewhere else
            Shader.SetGlobalFloat(Shader.PropertyToID("_RedTransmittance"), customEffect.redTransmittance.value);
            Shader.SetGlobalFloat(Shader.PropertyToID("_GreenTransmittance"), customEffect.greenTransmittance.value);
            Shader.SetGlobalFloat(Shader.PropertyToID("_BlueTransmittance"), customEffect.blueTransmittance.value);
            
            Shader.SetGlobalFloat(Shader.PropertyToID("_SunScale"), customEffect.sunScale.value);
            Shader.SetGlobalFloat(Shader.PropertyToID("_DensityScale"), customEffect.densityScale.value);
            Shader.SetGlobalFloat(Shader.PropertyToID("_MaxDistance"), customEffect.maxDistance.value);
